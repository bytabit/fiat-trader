...
import org.bytabit.ft.trade.model.{CertifyPaymentEvidence, Offer, SellOffer, TakenOffer}
...
object TradeWalletManager {
  ...
  case class TakeSellOffer(sellOffer: SellOffer, paymentDetails: String) extends Command
...
class TradeWalletManager extends WalletManager {
  ...
  when(RUNNING) {
    ...
    case Event(TakeSellOffer(sellOffer: SellOffer, paymentDetails: String), Data(k, wl, al)) =>
      Context.propagate(btcContext)
      val w = k.wallet
      val key = AESCipher.genRanData(AESCipher.AES_KEY_LEN)
      sender ! SellOfferTaken(sellOffer.take(paymentDetails, key)(w))
      stay()
