...
object TxTools {
  ...
  // TODO FT-30: user should be able to configure default transaction fee
...
trait TxTools {
  ...
  def sortedEscrowKeys(a: Arbitrator, s: BtcBuyer, b: Buyer): Seq[ECKey] =
    sortedEscrowKeys(List(a.escrowPubKey, b.escrowPubKey, s.escrowPubKey))

  def escrowPayoutScript(a: Arbitrator, s: BtcBuyer, b: Buyer): Script =
    ScriptBuilder.createP2SHOutputScript(2, sortedEscrowKeys(a, s, b))

  def escrowRedeemScript(a: Arbitrator, s: BtcBuyer, b: Buyer): Script =
    ScriptBuilder.createRedeemScript(2, sortedEscrowKeys(a, s, b))

  def escrowAddress(a: Arbitrator, s: BtcBuyer, b: Buyer) = escrowPayoutScript(a, s, b).getToAddress(a.netParams)
}